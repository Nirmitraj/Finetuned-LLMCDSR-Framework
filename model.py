...
Train a robust sequential recommender (SASRec)
Enhance it with LLM-based augmentation via Selector
Leverage domain-specific projections
Evaluate with ranking-based metrics (via Evaluator, which you already defined)
...
class Recaller(torch.autograd.Function):
    ...
    def forward(ctx, gen_embedding, item_embedding, k, data_embedding, Anum, is_B=False):
        ...
        # ✅ Final clamp to ensure safe indexing
        sim_rank = torch.clamp(sim_rank, 0, data_embedding.size(0) - 1)
        
        print(f"[DEBUG] data_embedding size: {data_embedding.size()}")
        print(f"[DEBUG] sim_rank max: {sim_rank.max().item()}, min: {sim_rank.min().item()}")
...
class Selector(torch.nn.Module):
    ...
    def forward(self, domains, gen_embedding, his_seqs, rec_model):
        ...
        assert batch_seq.min().item() >= 0, f"batch_seq has negative indices!"
        ...
        print(f"[DEBUG] batch_seq max: {batch_seq.max().item()}, item_emb size: {rec_model.item_emb.num_embeddings}")
        print(f"[DEBUG] batch_pos max: {batch_pos.max().item()}")
        print(f"[DEBUG] batch_neg max: {batch_neg.max().item()}")
    ...
    def _getitem(self, aug_data, fake_flag):
        ...
        # ✅ Clamp pos/neg to valid indices
        pos = np.clip(pos, 0, self.itemnum)
        neg = np.clip(neg, 0, self.itemnum)
...
class SASRec(torch.nn.Module):
    def __init__(self, user_num, Anum, Bnum, args):
        ...
        # uncomment below line for music-games dataset 
        # self.pos_emb = torch.nn.Embedding(args.maxlen, args.hidden_units) # TO IMPROVE

        #uncomment below line for pet-beauty dataset
        self.pos_emb = torch.nn.Embedding(args.maxlen + 1, args.hidden_units)
    ...
    def ensure_pos_emb_size(self, seq_len):
        if seq_len > self.pos_emb.num_embeddings:
            print(f"[WARNING] seq_len={seq_len} exceeds pos_emb size {self.pos_emb.num_embeddings}. Expanding pos_emb.")
            old_weight = self.pos_emb.weight.data
            new_pos_emb = torch.nn.Embedding(seq_len + 1, old_weight.shape[1]).to(self.dev)
            new_pos_emb.weight.data[:old_weight.shape[0]] = old_weight
    ...
    def log2feats(self, log_seqs, log_seqs_emb=None):
        ...
        # ✅ Dynamic resize just before embedding lookup
        self.ensure_pos_emb_size(log_seqs.shape[1])

        positions = np.clip(positions, 0, self.pos_emb.num_embeddings - 1)
    ...
    def shadow_forward(self, ori_seqs, aug_seqs, aug_embs):
        ...
        assert pos_seqs.max().item() < self.item_emb.num_embeddings, f"pos_seqs max index exceeds item_emb size!"
        assert log_seqs.max().item() < self.item_emb.num_embeddings, f"log_seqs max index exceeds item_emb size!"
        assert neg_seqs.max().item() < self.item_emb.num_embeddings, f"neg_seqs max index exceeds item_emb size!"
