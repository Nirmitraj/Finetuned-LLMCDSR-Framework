'''
This script forms the glue layer between your model and the data, providing:

Determinism
Loading of evaluation/test sets
Contrastive learning
Model checkpointing
Preprocessing for multi-domain recommendation
'''
import sys
import copy
import torch
...
#     file_path = f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/generation/cand_embeddings/{dataset}_generation_jina.pkl"
...
#             user_id = hash(user_id) % (10**8)
#             embed = torch.from_numpy(embed)
#             if domain == 1: 
#                 pass #user_id += usernum_A
#             if user_id not in train_users:
#                 continue  # ✅ Skip invalid IDs
#             source_seq = train_users[user_id]
#             res.append((domain, source_seq, embed))

#             file_contributions[domain] += 1  # Count how many samples per domain
#     print(f"[DEBUG] Generation contributions: {dict(file_contributions)}")

#     return res

def load_llm_generation(dataset, train_users, usernum_A, backbone):
    file_path = f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/tuned_generation/cand_embeddings/{dataset}_generation_jina.pkl"
    ...
    train_user_ids = list(train_users.keys())
    ...
    with open(file_path, 'rb') as f:
        ...
        for record in generation_data:
            ...
            # Map user_id as index into train_user_ids list
            if user_id >= len(train_user_ids):
                continue  # Skip invalid indices

            mapped_user_id = train_user_ids[user_id]

            source_seq = train_users[mapped_user_id]
            res.append((domain, source_seq, torch.from_numpy(embed)))

            file_contributions[domain] += 1
...
def load_item_embedding(dataset, device):
    ...
    A_path = f"/home/n.dholakia002/LLMCDSR/LLMCDSR/pre-train/experiments/{dataset}/llmcdsr-jina-A/best.pt"
    B_path = f"/home/n.dholakia002/LLMCDSR/LLMCDSR/pre-train/experiments/{dataset}/llmcdsr-jina-B/best.pt"
