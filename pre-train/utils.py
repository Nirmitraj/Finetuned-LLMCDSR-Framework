#6-Step : utils.py contains contrastive loss, evaluation loaders, early stopping, and embedding loaders. It fully supports training, validation, and embedding alignment, just as the LLMCDSR paper requires.
...
def load_llm_generation(dataset, train_users, usernum_A, embedding_dir):
    path = f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/embeddings/{dataset}_generation_jina.pkl"
...
def load_item_embedding(dataset, device, embedding_dir, projection=False):
    if not projection:
        A = np.load(f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/embeddings/A_item_jina.npy")
        B = np.load(f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/embeddings/B_item_jina.npy")
    else:
        key_ = 'pred_model.item_embeddings.weight'
        A = torch.load(f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/embeddings/{dataset}_projection_A.pt", map_location=device)[key_].data.cpu().to(device)
        B = torch.load(f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/embeddings/{dataset}_projection_B.pt", map_location=device)[key_].data.cpu().to(device)
    return A, B

def load_projection(dataset, device, embedding_dir):
    key_ = 'proj_model.projection.weight'
    
    A_path = f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/embeddings/{dataset}_projection_A.pt"
    B_path = f"/home/n.dholakia002/LLMCDSR/LLMCDSR/generation/embeddings/{dataset}_projection_B.pt"
    
    A = torch.load(A_path, map_location=device)[key_].data.cpu().to(device)
    B = torch.load(B_path, map_location=device)[key_].data.cpu().to(device)
